""" Char-based ConvNet toxic comment classifier.
...
# char_model.add(Dense(6))
# char_model.add(Activation('sigmoid'))

char_model = Model(inputs=x, outputs=out)
...
hist = char_model.fit(x_train, y_train,
...
print('\n##### ConvNet eval (train) #####\n')
yhat_train = np.round(char_model.predict(x_train)).astype('int32')
...
print('\n##### ConvNet eval (val) #####\n')
yhat_val = np.round(char_model.predict(x_val)).astype('int32')
...
print('\n##### ConvNet eval (test) #####\n')
yhat_test = np.round(char_model.predict(x_test)).astype('int32')
...
test_data = pad_sequences(test_sequences,
                          ...
                          padding='pre',
                          truncating='pre')
preds = char_model.predict(test_data)
predictions = zip(test_ids, preds)
now_str = datetime.now().strftime("%Y%m%d_%H%M")
f_name = "test_submission.char_cnn.%s.csv" % now_str
